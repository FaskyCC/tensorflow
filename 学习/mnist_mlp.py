...
with tf.Session() as sess:
    ...
    for i in range(3000+1):
        ...
        # 训练完后直接加载测试集数据，进行测试
        if i % 100 == 0:
            loss_run = sess.run(cross_entropy, feed_dict={xs: batch_xs, ys: batch_ys, keep_prob: 0.75})
            accuracy_run = sess.run(accuracy, feed_dict={xs: batch_xs, ys: batch_ys, keep_prob: 0.75})
